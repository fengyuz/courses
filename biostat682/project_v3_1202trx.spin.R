...
model.jags <- function(){
    for(i in 1:n){
        ...
        r[i] <- exp(beta[i,3])
        s[i] <- exp(beta[i,4])
        beta[i,1:4] ~ dmnorm(c(0,0,0,0), eye.four)
        ## beta[i,1:4] ~ dmnorm(beta.mu[(z[i]+1), 1:4], Omega[(z[i]+1), 1:4, 1:4])
    }
    tausq ~ dgamma(1e-1,1e-1)
    p ~ dbeta(1,1)
##     beta.mu[1,1] ~ dunif(-5, 20)
##     beta.mu[1,2] ~ dunif(-10, 3)
##     beta.mu[1,3] ~ dunif(-10, 3)
##     beta.mu[1,4] ~ dunif(-10, 3)
##     Omega[1, 1:4, 1:4] ~ dwish(eye.four, 5)
...
##      n.chains=2,
##     model.file=model.JAGS,
##     jags.module = c("mix", "glm","dic")
## )
Sys.time()
save(fit.jags, file=paste0("fit.jags.RData.", toString(Sys.time())))
## rm(list=ls())
## load("fit.JAGS.RData")
## fit.JAGS.mcmc <- as.mcmc(fit.JAGS)
## print(gelman.diag(fit.JAGS.mcmc))
fit.jags.mcmc <- as.mcmc(fit.jags)
print(gelman.diag(fit.jags.mcmc, multivariate = FALSE))
## Sys.time()
