...
# dose <- c(.05,.1,.2,.3,.5,.7)            
dose<- c(0.05, 0.1, 0.2, 0.3, 0.5)              
dose  <- log(dose/(1-dose)) - 3
Fstar <- 0.3                             # target tox

## assumed scenarios
# p1    <-  c(0.05, 0.15, 0.3, 0.45, 0.6)     # scenario 1 
p1    <- c(0.05, 0.10, 0.2, 0.30, 0.5)
# p1    <- c(.05, .1, .2, .3, .5, .7)      # scenario 1 
# p1    <- c(.05, .2, .3, .4, .6, .8)      # scenario 1 
...
sim.trial <- function(nmax=25,p0=p1)
  { ## simulate one realization of the trial
    ...
    repeat{
      ...
      d.count <- unname(table(factor(dta[,1], lev=1:5)))[d]
      if (n >= nmax | (n >= 18 & d.count > 6))
        break
      uk <-   sim.uk(p0,d,k=k)  # next batch
...
Ftox <- function(d,beta)
  { ## returns (model) prob of toxicity F(d,beta)
    # return(dose[d]^beta)
  return(exp(3 + beta * dose[d]) / (1 + exp(3 + beta * dose[d])))
...
Ftoxb <- function(beta)
  ...
  # return(dose^beta)
  return(exp(3 + beta * dose) / (1 + exp(3 + beta * dose)))
}
...
n.max <- 30
for(i in 1:nSim){
  trial <- sim.trial(nmax=n.max, p0=p1)
  dta <- data.frame(trial$dta)
  n <- nrow(dta)
  dta$dlt <- (dta$uk < Th)
  num.pat <- unname(table(factor(dta$d, lev = 1:5)))
  prop.dlt <- sum(dta$uk < Th) / n
  dta.agg <- aggregate(dta, by = list(dta$d), FUN = sum)
  tox <- rep(0, 5)
  d.pos <- unique(dta.agg$Group.1)
  for(i in 1:length(d.pos)){
    tox[d.pos[i]] = dta.agg$dlt[i]
  }
  result <- rbind(result, c(trial$dstar, num.pat, prop.dlt, n, tox))
}
...
tox <- colSums(result[,9:13]) / colSums(result[,2:6])
