...
phaseIsim <- function(nsim, npat, sdose, prob.tox, design='CRM', ff=2, prior.theta=c(1, 1, 1), 
        ...
        burnin.itr=1000, production.itr=1000, nseed=10, outfile="fileout")
{
    set.seed(nseed)    
    k <- length(sdose)    
    work <- list()
        ...
        ff, prior.theta, out.file, theLot=1, stop.crit=6, crm.min=18, burnin.itr=1000, production.itr=1000)
{
    ...
    {              
        ...
        ans <- ewoc.nextdose(work$dose, ff, pdes, sampled.data)        
        ix <- ans[1]
        mean.theta <- ans[2]
...
#                     
# ----------------------------------------------------------------------
ewoc.nextdose <- function(Dose, ff, pdes, sample.theta, ewocpar = c(0.25, 0.3, 0.05)) {
    k <- length( Dose )    
    alpha <- ewocpar[1]
    t1 <- ewocpar[2]
    t2 <- ewocpar[3]
    ...
    # dose.ok <- Dose[Dose <= x0 + t1 & Dose <= alpha + t2]
    # ix <- which.is.max(dose.ok)
    ix <- order( abs(Dose-x0))[1]   # an approximation 
...
library(BRugs)
library(nnet)
setwd("~/biostat830/finalProject/phaseIsim")
# p.tox0 <- c(.05,.15,.3,.45,.6)
p.tox0 <- c(.05,.10,.2,.30,.5)
...
# 3+3 method
# phaseIsim(nsim=1000, npat=30, sdose=s.dose, prob.tox=p.tox0, design='3+3', outfile='3plus3.txt')

######################
#  THESE 3+3 RESULTS FORM THE FIRST PART OF SCENARIO 1, TABLE 3.2, BCLM pg.100
#  TO CREATE THE REST OF SCENARIO 1:

#  (NOTE: 1000 CRM1 sims take about 50 mins on a 2007 laptop!)
######################
...
# # CRM 1:
# loopstart <- Sys.time()
# phaseIsim(nsim=1000, npat=30, sdose=s.dose, prob.tox=p.tox0, outfile='CRM1.txt')
# tottime <- Sys.time() - loopstart
# cat("\n\nTotal time elapsed: "); print(tottime)

# CRM 3:
loopstart <- Sys.time()
phaseIsim(nsim=1000, npat=30, sdose=s.dose, prob.tox=p.tox0, crm.group.size=3, outfile='CRM3_ewoc.txt')
tottime <- Sys.time() - loopstart
cat("\n\nTotal time elapsed: "); print(tottime)
